...
export class ReportsController {
  ...
  @Post('export-groups')
  async exportGroupsReport(@Body() body: { grupos: any[] }, @Res() res: Response) {
    try {
      const buffer = await this.reportsService.generateGroupsExcel(body.grupos);
      
      res.set({
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Reporte_Grupos_${Date.now()}.xlsx"`,
        'Content-Length': buffer.length.toString(),
      });
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ 
        message: error.message || 'Error al generar excel de grupos' 
      });
    }
  }
